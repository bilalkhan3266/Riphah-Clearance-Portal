...
import {
  ...
  RiUserSettingsLine, RiLogoutBoxLine, RiBookOpenLine, RiAlertFill, 
...
export default function Phase1Dashboard() {
  ...
  const displayName = user?.full_name || "Phase 1 Staff";
    ...
    <div className="flex h-screen bg-gray-50 font-sans text-gray-800 overflow-hidden">
      <aside className="w-[280px] flex flex-col bg-gradient-to-b from-[#0a0f24] via-[#1b2a56] to-[#182848] text-white py-6 px-4 shadow-xl overflow-y-auto shrink-0">
        <div className="flex items-center gap-3 mb-8 pb-5 border-b border-white/10">
          ...
          <h1 className="text-lg font-bold bg-gradient-to-r from-blue-300 to-indigo-300 bg-clip-text text-transparent">Phase 1</h1>
        </div>
        ...
        <div className="flex items-center gap-3 mb-8 p-4 rounded-xl bg-white/5 border border-white/10">
          ...
          <div className="min-w-0">
            ...
            <p className="text-[11px] text-gray-300 truncate">{displaySap} • Phase 1</p>
          </div>
        </div>
        ...
        <nav className="flex flex-col gap-1 flex-1">
          <button
            ...
            className={`flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-all duration-200 ${activeTab === "pending" ? "bg-gradient-to-r from-blue-500 to-indigo-600 text-white shadow-lg shadow-blue-500/20" : "text-gray-300 hover:bg-white/10 hover:text-white"}`}
          >
          ...
          <button onClick={() => navigate("/phase1-messages")} className="flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium text-gray-300 hover:bg-white/10 hover:text-white transition-all duration-200">
            <RiMessage2Line size={18} /> Messages
          </button>
          <button onClick={() => navigate("/phase1-edit-profile")} className="flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium text-gray-300 hover:bg-white/10 hover:text-white transition-all duration-200">
      ...
      <main className="flex-1 overflow-y-auto p-6 lg:p-8">
        <div className="mb-8 rounded-2xl bg-gradient-to-br from-blue-50 to-indigo-50 border border-blue-100 shadow-sm p-6 hover:shadow-lg transition-all duration-300">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-extrabold text-gray-900 tracking-tight">Phase 1 Clearance Management</h1>
                ...
                <tbody className="divide-y divide-gray-100">
                  {requests.map((req, index) => (
                    <tr key={req._id || req.id} className="hover:bg-blue-50/30 transition-colors duration-150">
                      ...
                      <td className="px-5 py-4 text-gray-600 text-xs max-w-[200px] truncate">{req.remarks || req.message || "—"}</td>
                      <td className="px-5 py-4 text-gray-500 text-xs whitespace-nowrap">{new Date(req.created_at || req.createdAt).toLocaleDateString()}</td>
                      {activeTab === "pending" && (
