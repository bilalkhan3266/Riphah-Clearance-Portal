...
import { MdCheckCircle, MdMail, MdLogout, MdSend, MdReply, MdThumbUp, MdCancel, MdDownload, MdAttachFile, MdDescription, MdRefresh, MdPushPin, MdAssignment, MdChat, MdClose, MdCheck, MdHome } from 'react-icons/md';
import axios from 'axios';
import '../../../Departments/Phase1/Library/LibraryClearance.css';

export default function WardenClearanceEnhanced({ onComplete }) {
  ...
  const DEPARTMENT = 'Warden';

  // Fetch clearance requests and messages on mount
  useEffect(() => {
    console.log('Warden Component Mounted - Fetching clearance requests...');
  ...
  const fetchAllClearanceRequests = async () => {
    try {
      ...
      const response = await axios.get(`${API_URL}/api/clearance-requests?department=Warden`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (response.data.success && response.data.requests) {
        const requests = response.data.requests;
        
        // Filter for PENDING Warden approvals (no Warden status or not Approved/Rejected)
        const pending = requests.filter(req => 
          !req.departments?.Warden || 
          (req.departments?.Warden?.status !== 'Approved' && 
           req.departments?.Warden?.status !== 'Rejected')
        );
        setPendingRequests(pending);
        
        // Filter for APPROVED Warden requests
        const approved = requests.filter(req => 
          req.departments?.Warden?.status === 'Approved'
        );
        setApprovedRequests(approved);
        
        // Filter for REJECTED Warden requests
  ...
  const handleDownloadApprovalLetter = (request, approvalType) => {
    try {
      ...
      const date = new Date(request.departments?.Warden?.checked_at).toLocaleDateString();
...
RE: WARDEN CLEARANCE APPROVAL

Dear ${facultyName},

This is to certify that your clearance request for Warden Department has been APPROVED.

Status: APPROVED
Approved By: ${request.departments?.Warden?.approved_by || 'Warden Staff'}
Remarks: ${request.departments?.Warden?.remarks || 'Warden clearance completed successfully'}
...
RE: WARDEN CLEARANCE REJECTION

Dear ${facultyName},

Your clearance request for Warden Department has been REJECTED and requires further action.

Status: REJECTED
Rejected By: ${request.departments?.Warden?.rejected_by || 'Warden Staff'}
Reason: ${request.departments?.Warden?.remarks || 'Requirements not met'}
...
Riphah International University
      ...
      link.href = url;
      link.setAttribute('download', `Warden_${approvalType === 'approved' ? 'Approval' : 'Rejection'}_${facultyName}_${date}.txt`);
  ...
  const displayName = user?.full_name || 'Warden Staff';
    ...
    <div className="department-page">
      <aside className="fd-sidebar">
        <div className="fd-profile">
          ...
          <div>
            ...
            <p className="fd-small">{displayRole} • Warden Department</p>
      ...
      <main className="fd-main">
        <header className="fd-header">
          <div>
            <h1><MdHome /> Warden Clearance Processing</h1>
        ...
        {activeTab === 'faculty-list' && (
          <div className="faculty-list-section">
            <div className="section-header">
              ...
              <p>Select a faculty to process their warden clearance</p>
            </div>
                ...
                <div>
                  ...
                  <div className="library-clearance-form">
                    <div className="form-header">
                      <h3><MdHome /> Warden Clearance Processing</h3>
        ...
        {activeTab === 'approved' && (
          <div className="approved-section">
            <div className="section-header">
              ...
              <p>Faculty members with approved warden clearance</p>
            </div>
                  ...
                  <div key={request._id} className="clearance-card approved">
                    ...
                    <div className="faculty-info-card">
                      ...
                      <div className="approval-details">
                        <p><strong>Approved Date:</strong> {new Date(request.departments?.Warden?.checked_at).toLocaleDateString()}</p>
                        <p><strong>Approved By:</strong> {request.departments?.Warden?.approved_by || 'Warden Staff'}</p>
                        <p><strong>Remarks:</strong> {request.departments?.Warden?.remarks || 'Warden clearance approved'}</p>
                      </div>
        ...
        {activeTab === 'rejected' && (
          <div className="rejected-section">
            <div className="section-header">
              ...
              <p>Faculty members with rejected warden clearance</p>
            </div>
                  ...
                  <div key={request._id} className="clearance-card rejected">
                    ...
                    <div className="faculty-info-card">
                      ...
                      <div className="rejection-details">
                        <p><strong>Rejection Date:</strong> {new Date(request.departments?.Warden?.checked_at).toLocaleDateString()}</p>
                        <p><strong>Rejected By:</strong> {request.departments?.Warden?.rejected_by || 'Warden Staff'}</p>
                        <p><strong>Reason:</strong> {request.departments?.Warden?.remarks || 'Warden clearance not met'}</p>
                      </div>
        ...
        {activeTab === 'issues' && (
          <div className="issues-section">
            <div className="section-header">
              <h2><MdPushPin /> Create Issues for Warden Department</h2>
            ...
            {departmentIssues.length > 0 ? (
              ...
              <div className="empty-state">
                <p><MdCheckCircle /> No pending issues for Warden department</p>
              </div>
            )}
          </div>
        )}
        ...
        {activeTab === 'returns' && (
          <div className="returns-section">
            <div className="section-header">
              <h2><MdCheckCircle /> Record Returns for Warden Department</h2>
            ...
            {departmentReturns.length > 0 ? (
              ...
              <div className="empty-state">
                <p>No returned items yet for Warden department</p>
              </div>
            )}
          </div>
        )}
