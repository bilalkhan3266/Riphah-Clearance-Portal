...
import { MdLocalLibrary, MdAssignment, MdPendingActions, MdCheckCircle, MdSend } from 'react-icons/md';
...
export default function LibraryIssue() {
  ...
  const fetchIssues = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`/api/issues/pending/Library`);
  ...
  return (
    <div className="issue-container">
      <div className="issue-header">
        <h2><MdLocalLibrary style={{verticalAlign:'middle',marginRight:'8px'}} /> Library - Issue Files</h2>
        ...
        <form onSubmit={handleIssueItem} className="issue-form">
          ...
          <div className="form-group">
            ...
            <input
              ...
              onChange={handleFormChange}
              placeholder="Enter item name (e.g., Books, Journals)"
              required
            />
          </div>
