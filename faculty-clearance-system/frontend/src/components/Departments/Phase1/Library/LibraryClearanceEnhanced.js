...
import { MdCheckCircle, MdMail, MdLogout, MdSend, MdReply, MdCancel, MdAssignment, MdHourglassEmpty, MdClose, MdRefresh, MdPushPin, MdMenuBook } from 'react-icons/md';
import axios from 'axios';
import './LibraryClearance.css';

export default function LibraryClearanceEnhanced({ onComplete }) {
  ...
  const DEPARTMENT = 'Library';

  // Fetch clearance requests and messages on mount
  useEffect(() => {
    console.log('Library Component Mounted - Fetching clearance requests...');
  ...
  const fetchAllClearanceRequests = async () => {
    try {
      ...
      const response = await axios.get(`${API_URL}/api/clearance-requests?department=Library`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (response.data.success && response.data.requests) {
        const requests = response.data.requests;
        
        // Filter for PENDING Library approvals (no Library status or not Approved/Rejected)
        const pending = requests.filter(req => 
          !req.departments?.Library || 
          (req.departments?.Library?.status !== 'Approved' && 
           req.departments?.Library?.status !== 'Rejected')
        );
        setPendingRequests(pending);
        
        // Filter for APPROVED Library requests
        const approved = requests.filter(req => 
          req.departments?.Library?.status === 'Approved'
        );
        setApprovedRequests(approved);
        
        // Filter for REJECTED Library requests
  ...
  const handleDownloadApprovalLetter = (request, approvalType) => {
    try {
      ...
      const date = new Date(request.departments?.Library?.checked_at).toLocaleDateString();
...
RE: LIBRARY CLEARANCE APPROVAL

Dear ${facultyName},

This is to certify that your clearance request for Library Department has been APPROVED.

Status: APPROVED
Approved By: ${request.departments?.Library?.approved_by || 'Library Staff'}
Remarks: ${request.departments?.Library?.remarks || 'Library clearance completed successfully'}
...
RE: LIBRARY CLEARANCE REJECTION

Dear ${facultyName},

Your clearance request for Library Department has been REJECTED and requires further action.

Status: REJECTED
Rejected By: ${request.departments?.Library?.rejected_by || 'Library Staff'}
Reason: ${request.departments?.Library?.remarks || 'Requirements not met'}
...
Riphah International University
      ...
      link.href = url;
      link.setAttribute('download', `Library_${approvalType === 'approved' ? 'Approval' : 'Rejection'}_${facultyName}_${date}.txt`);
    ...
    <div className="department-page">
      <aside className="fd-sidebar">
        <div className="fd-profile">
          ...
          <div>
            ...
            <p className="fd-small">{displayRole} • Library Department</p>
      ...
      <main className="fd-main">
        <header className="fd-header">
          <div>
            <h1><MdMenuBook style={{verticalAlign:'middle',marginRight:'8px'}} /> Library Clearance Processing</h1>
        ...
        {activeTab === 'faculty-list' && (
          <div className="faculty-list-section">
            <div className="section-header">
              ...
              <p>Select a faculty to process their library clearance</p>
            </div>
                ...
                <div>
                  ...
                  <div className="library-clearance-form">
                    <div className="form-header">
                      <h3><MdMenuBook style={{verticalAlign:'middle',marginRight:'8px'}} /> Automatic Library Clearance Decision</h3>
                      <span className="phase-badge">Phase 1 - Physical Assets (25%)</span>
                    </div>

                    {/* Automatic Decision Display */}
                    <div style={{
                      padding: '20px',
                      backgroundColor: selected.departments?.Library?.status === 'Approved' ? '#d1fae5' : '#fee2e2',
                      border: `2px solid ${selected.departments?.Library?.status === 'Approved' ? '#10b981' : '#ef4444'}`,
                      borderRadius: '8px',
                      marginTop: '16px'
                    }}>
                      <div style={{ textAlign: 'center', marginBottom: '16px' }}>
                        {selected.departments?.Library?.status === 'Approved' ? (
                          <div>
                            <h2 style={{ color: '#047857', margin: '0 0 8px 0' }}><MdCheckCircle style={{verticalAlign:'middle',marginRight:'4px'}} /> APPROVED</h2>
                            <p style={{ color: '#059669' }}>No pending items - Faculty is cleared for Library Department</p>
                          </div>
                        ) : (
                          <div>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginTop: '12px' }}>
                        <div style={{ padding: '12px', backgroundColor: 'white', borderRadius: '6px' }}>
                          <strong>Status:</strong> {selected.departments?.Library?.status || 'Pending'}
                        </div>
                        <div style={{ padding: '12px', backgroundColor: 'white', borderRadius: '6px' }}>
                          <strong>Last Checked:</strong> {selected.departments?.Library?.checked_at 
                            ? new Date(selected.departments?.Library?.checked_at).toLocaleDateString() 
                            : 'Not checked'}
                        </div>
                        <div style={{ padding: '12px', backgroundColor: 'white', borderRadius: '6px', gridColumn: '1 / -1' }}>
                          <strong>Reason:</strong> {selected.departments?.Library?.remarks || 'Awaiting return submission and verification'}
                        </div>
                      </div>
                    </div>
          ...
          <div className="issues-section">
            ...
            {departmentIssues.length > 0 ? (
              ...
              <div className="empty-state" style={{
                ...
                <p style={{ fontSize: '18px' }}><MdCheckCircle style={{verticalAlign:'middle',marginRight:'4px',color:'#10b981'}} /> No pending issues for Library department</p>
              </div>
            )}
          </div>
        )}
                ...
                {departmentReturns.map((item) => (
                  <div key={item._id} className="return-card" style={{
                    ...
                    <div className="return-details" style={{ marginTop: '12px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                      ...
                      <p style={{ margin: '0' }}><strong>Received By:</strong> {item.receivedBy || 'Library Staff'}</p>
                    </div>
                  </div>
                ))}
              </div>
