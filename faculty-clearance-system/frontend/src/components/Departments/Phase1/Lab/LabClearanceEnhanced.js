...
import { MdCheckCircle, MdMail, MdLogout, MdSend, MdReply, MdThumbUp, MdCancel, MdDownload, MdAttachFile, MdDescription, MdAssignment, MdHourglassEmpty, MdClose, MdRefresh, MdPushPin, MdScience, MdChecklistRtl, MdError, MdCheckBox, MdMoreVert, MdEditNote, MdRemove, MdAdd, MdInfo, MdDoneAll } from 'react-icons/md';
import axios from 'axios';
import './LabClearance.css';

export default function LabClearanceEnhanced({ onComplete }) {
  ...
  useEffect(() => {
    console.log('Lab Component Mounted - Fetching clearance requests...');
  ...
  const fetchAllClearanceRequests = async () => {
    try {
      ...
      const response = await axios.get(`${API_URL}/api/clearance-requests?department=Lab`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (response.data.success && response.data.requests) {
        const requests = response.data.requests;
        
        // Filter for PENDING Lab approvals (no Lab status or not Approved/Rejected)
        const pending = requests.filter(req => 
          !req.departments?.Lab || 
          (req.departments?.Lab?.status !== 'Approved' && 
           req.departments?.Lab?.status !== 'Rejected')
        );
        setPendingRequests(pending);
        
        // Filter for APPROVED Lab requests
        const approved = requests.filter(req => 
          req.departments?.Lab?.status === 'Approved'
        );
        setApprovedRequests(approved);
        
        // Filter for REJECTED Lab requests
  ...
  const handleSelectFaculty = (facultyId) => {
    // Extract the faculty ID string if an object is passed
    ...
    try {
      ...
      const response = await axios.post(
        `${API_URL}/api/clearance-requests/${selectedFacultyId}/approve`,
        {
          ...
          remarks: remarks || 'Lab clearance completed',
  ...
  const handleDownloadApprovalLetter = (request, approvalType) => {
    try {
      ...
      const date = new Date(request.departments?.Lab?.checked_at).toLocaleDateString();
...
RE: LAB CLEARANCE APPROVAL

Dear ${facultyName},

This is to certify that your clearance request for Lab Department has been APPROVED.

Status: APPROVED
Approved By: ${request.departments?.Lab?.approved_by || 'Lab Staff'}
Remarks: ${request.departments?.Lab?.remarks || 'Lab clearance completed successfully'}
...
RE: LAB CLEARANCE REJECTION

Dear ${facultyName},

Your clearance request for Lab Department has been REJECTED and requires further action.

Status: REJECTED
Rejected By: ${request.departments?.Lab?.rejected_by || 'Lab Staff'}
Reason: ${request.departments?.Lab?.remarks || 'Requirements not met'}
...
Riphah International University
      ...
      link.href = url;
      link.setAttribute('download', `Lab_${approvalType === 'approved' ? 'Approval' : 'Rejection'}_${facultyName}_${date}.txt`);
  ...
  const displayName = user?.full_name || 'Lab Staff';
    ...
    <div className="department-page">
      <aside className="fd-sidebar">
        <div className="fd-profile">
          ...
          <div>
            ...
            <p className="fd-small">{displayRole} • Lab Department</p>
        ...
        <nav className="fd-nav">
          ...
          <button onClick={() => setActiveTab('faculty-list')} className={`fd-nav-btn ${activeTab === 'faculty-list' ? 'active' : ''}`}>
            <MdChecklistRtl className="nav-icon" /> Faculty List
          ...
          <button onClick={() => setActiveTab('issues')} className={`fd-nav-btn ${activeTab === 'issues' ? 'active' : ''}`}>
            <MdError className="nav-icon" /> Create Issues
          ...
          <button onClick={() => setActiveTab('returns')} className={`fd-nav-btn ${activeTab === 'returns' ? 'active' : ''}`}>
            <MdCheckBox className="nav-icon" /> Record Returns
      ...
      <main className="fd-main">
        <header className="fd-header">
          <div>
            <h1><MdScience style={{verticalAlign:'middle',marginRight:'12px',fontSize:'32px',color:'#3b82f6'}} /> Lab Clearance Processing</h1>
            <p style={{display:'flex',alignItems:'center',gap:'8px'}}><MdInfo style={{fontSize:'16px',color:'#0ea5e9'}} /> Manage faculty clearance requests and laboratory item returns</p>
          </div>
        </header>
        ...
        {activeTab === 'faculty-list' && (
          <div className="faculty-list-section">
            <div className="section-header">
              <h2><MdChecklistRtl style={{verticalAlign:'middle',marginRight:'12px',fontSize:'24px',color:'#3b82f6'}} /> Faculty Members</h2>
              <p><MdInfo style={{verticalAlign:'middle',marginRight:'6px',fontSize:'16px'}} /> Select a faculty to process their lab clearance</p>
            </div>
            ...
            {pendingRequests.length > 0 ? (
              ...
              <div className="empty-state">
                <p><MdDoneAll style={{verticalAlign:'middle',marginRight:'8px',fontSize:'28px',color:'#10b981'}} /> All faculty cleared! No pending requests.</p>
              </div>
            )}
          </div>
        )}

        {/* SELECTED FACULTY TAB - SHOW AUTOMATIC DECISION */}
                ...
                <div>
                  ...
                  <div className="library-clearance-form">
                    <div className="form-header">
                      <h3><MdScience style={{verticalAlign:'middle',marginRight:'10px',fontSize:'20px',color:'#3b82f6'}} /> Automatic Lab Clearance Decision</h3>
                      <span className="phase-badge">📚 Phase 1 - Laboratory Services</span>
                    </div>

                    {/* Automatic Decision Display */}
                    <div style={{
                      padding: '20px',
                      backgroundColor: selected.departments?.Lab?.status === 'Approved' ? '#d1fae5' : '#fee2e2',
                      border: `2px solid ${selected.departments?.Lab?.status === 'Approved' ? '#10b981' : '#ef4444'}`,
                      borderRadius: '8px',
                      marginTop: '16px'
                    }}>
                      <div style={{ textAlign: 'center', marginBottom: '16px' }}>
                        {selected.departments?.Lab?.status === 'Approved' ? (
                          <div>
                            <h2 style={{ color: '#047857', margin: '0 0 8px 0',fontSize:'28px',display:'flex',alignItems:'center',justifyContent:'center',gap:'8px' }}><MdCheckCircle style={{fontSize:'32px'}} /> APPROVED</h2>
                            <p style={{ color: '#059669',fontSize:'15px' }}>✓ No pending items - Faculty is cleared for Lab Department</p>
                          </div>
                        ) : (
                          <div>
                            <h2 style={{ color: '#991b1b', margin: '0 0 8px 0',fontSize:'28px',display:'flex',alignItems:'center',justifyContent:'center',gap:'8px' }}><MdHourglassEmpty style={{fontSize:'32px'}} /> PENDING</h2>
                            <p style={{ color: '#b91c1c',fontSize:'15px' }}>⏳ Faculty has items to return to complete clearance</p>
                          </div>
                        )}
                      </div>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginTop: '12px' }}>
                        <div style={{ padding: '12px', backgroundColor: 'white', borderRadius: '6px' }}>
                          <strong>Status:</strong> {selected.departments?.Lab?.status || 'Pending'}
                        </div>
                        <div style={{ padding: '12px', backgroundColor: 'white', borderRadius: '6px' }}>
                          <strong>Last Checked:</strong> {selected.departments?.Lab?.checked_at 
                            ? new Date(selected.departments?.Lab?.checked_at).toLocaleDateString() 
                            : 'Not checked'}
                        </div>
                        <div style={{ padding: '12px', backgroundColor: 'white', borderRadius: '6px', gridColumn: '1 / -1' }}>
                          <strong>Reason:</strong> {selected.departments?.Lab?.remarks || 'Awaiting return submission and verification'}
                        </div>
                      </div>
                    </div>

                    <p style={{ marginTop: '16px', padding: '12px', backgroundColor: '#f0f9ff', border: '1px solid #bfdbfe', borderRadius: '6px', color: '#0c4a6e' }}>
                      <strong>Note:</strong> This decision is made automatically by the system based on pending returns. 
                      If faculty has returned all items, approval is automatic. Use the "Issues" tab to create issues for unreturned items.
                    </p>
        ...
        {activeTab === 'messages' && (
          <div className="messages-section">
            <div className="section-header">
              <h2><MdMail style={{verticalAlign:'middle',marginRight:'12px',fontSize:'24px',color:'#3b82f6'}} /> Messages</h2>
              <p><MdInfo style={{verticalAlign:'middle',marginRight:'6px',fontSize:'16px'}} /> Direct communication with faculty members</p>
            </div>
        ...
        {activeTab === 'issues' && (
          <div className="issues-section">
            <div className="section-header">
              <h2><MdError style={{verticalAlign:'middle',marginRight:'12px',fontSize:'24px',color:'#f59e0b'}} /> Issue Management</h2>
              <p><MdInfo style={{verticalAlign:'middle',marginRight:'6px',fontSize:'16px'}} /> Create and manage issues (items faculty need to return)</p>
            </div>
            ...
            <div style={{ marginBottom: '24px' }}>
              <button 
                ...
                style={{
                  padding: '12px 24px',
                  backgroundColor: showIssueForm ? '#ef4444' : '#3b82f6',
                  color: 'white',
                  border: 'none',
                  borderRadius: '8px',
                  cursor: 'pointer',
              ...
              >
                {showIssueForm ? <><MdClose style={{fontSize:'18px'}} /> Cancel</> : <><MdAdd style={{fontSize:'18px'}} /> Create New Issue</>}
              </button>
            ...
            {departmentIssues.length > 0 ? (
              ...
              <div className="empty-state" style={{
                ...
                <p style={{ fontSize: '18px' }}><MdCheckCircle style={{verticalAlign:'middle',marginRight:'4px',color:'#10b981'}} /> No pending issues for Lab department</p>
              </div>
            )}
          </div>
        )}
        ...
        {activeTab === 'returns' && (
          <div className="returns-section">
            <div className="section-header">
              <h2><MdCheckBox style={{verticalAlign:'middle',marginRight:'12px',fontSize:'24px',color:'#10b981'}} /> Return Management</h2>
              <p><MdInfo style={{verticalAlign:'middle',marginRight:'6px',fontSize:'16px'}} /> Record items returned by faculty members</p>
            </div>
            ...
            <div style={{ marginBottom: '24px' }}>
              <button 
                ...
                style={{
                  padding: '12px 24px',
                  backgroundColor: showReturnForm ? '#ef4444' : '#10b981',
                  color: 'white',
                  border: 'none',
                  borderRadius: '8px',
                  cursor: 'pointer',
              ...
              >
                {showReturnForm ? <><MdClose style={{fontSize:'18px'}} /> Cancel</> : <><MdAdd style={{fontSize:'18px'}} /> Record Return</>}
              </button>
                ...
                {departmentReturns.map((item) => (
                  <div key={item._id} className="return-card" style={{
                    ...
                    <div className="return-details" style={{ marginTop: '12px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                      ...
                      <p style={{ margin: '0' }}><strong>Received By:</strong> {item.receivedBy || 'Lab Staff'}</p>
                    </div>
                  </div>
                ))}
              </div>
