...
import { MdCheckCircle, MdMail, MdLogout, MdSend, MdReply, MdThumbUp, MdCancel, MdDownload, MdAttachFile, MdDescription, MdRefresh, MdPushPin, MdAssignment, MdChat, MdClose, MdCheck, MdComputer } from 'react-icons/md';
import axios from 'axios';
import '../../../Departments/Phase1/Library/LibraryClearance.css';

export default function ITClearanceEnhanced({ onComplete }) {
  ...
  useEffect(() => {
    console.log('IT Component Mounted - Fetching clearance requests...');
  ...
  const fetchAllClearanceRequests = async () => {
    try {
      ...
      const response = await axios.get(`${API_URL}/api/clearance-requests?department=IT`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (response.data.success && response.data.requests) {
        const requests = response.data.requests;
        
        // Filter for PENDING IT approvals (no IT status or not Approved/Rejected)
        const pending = requests.filter(req => 
          !req.departments?.IT || 
          (req.departments?.IT?.status !== 'Approved' && 
           req.departments?.IT?.status !== 'Rejected')
        );
        setPendingRequests(pending);
        
        // Filter for APPROVED IT requests
        const approved = requests.filter(req => 
          req.departments?.IT?.status === 'Approved'
        );
        setApprovedRequests(approved);
        
        // Filter for REJECTED IT requests
  ...
  const handleDownloadApprovalLetter = (request, approvalType) => {
    try {
      ...
      const date = new Date(request.departments?.IT?.checked_at).toLocaleDateString();
...
RE: IT CLEARANCE APPROVAL

Dear ${facultyName},

This is to certify that your clearance request for IT Department has been APPROVED.

Status: APPROVED
Approved By: ${request.departments?.IT?.approved_by || 'IT Staff'}
Remarks: ${request.departments?.IT?.remarks || 'IT clearance completed successfully'}
...
RE: IT CLEARANCE REJECTION

Dear ${facultyName},

Your clearance request for IT Department has been REJECTED and requires further action.

Status: REJECTED
Rejected By: ${request.departments?.IT?.rejected_by || 'IT Staff'}
Reason: ${request.departments?.IT?.remarks || 'Requirements not met'}
...
Riphah International University
      ...
      link.href = url;
      link.setAttribute('download', `IT_${approvalType === 'approved' ? 'Approval' : 'Rejection'}_${facultyName}_${date}.txt`);
  ...
  const displayName = user?.full_name || 'IT Staff';
    ...
    <div className="department-page">
      <aside className="fd-sidebar">
        <div className="fd-profile">
          ...
          <div>
            ...
            <p className="fd-small">{displayRole} • IT Department</p>
      ...
      <main className="fd-main">
        <header className="fd-header">
          <div>
            <h1><MdComputer /> IT Clearance Processing</h1>
        ...
        {activeTab === 'faculty-list' && (
          <div className="faculty-list-section">
            <div className="section-header">
              ...
              <p>Select a faculty to process their IT clearance</p>
            </div>
                ...
                <div>
                  ...
                  <div className="library-clearance-form">
                    <div className="form-header">
                      <h3><MdComputer /> IT Clearance Processing</h3>
                      <span className="phase-badge">Phase 3 - Systems & Access (75%)</span>
                    </div>
        ...
        {activeTab === 'approved' && (
          <div className="approved-section">
            <div className="section-header">
              ...
              <p>Faculty members with approved IT clearance</p>
            </div>
                  ...
                  <div key={request._id} className="clearance-card approved">
                    ...
                    <div className="faculty-info-card">
                      ...
                      <div className="approval-details">
                        <p><strong>Approved Date:</strong> {new Date(request.departments?.IT?.checked_at).toLocaleDateString()}</p>
                        <p><strong>Approved By:</strong> {request.departments?.IT?.approved_by || 'IT Staff'}</p>
                        <p><strong>Remarks:</strong> {request.departments?.IT?.remarks || 'IT clearance approved'}</p>
                      </div>
        ...
        {activeTab === 'rejected' && (
          <div className="rejected-section">
            <div className="section-header">
              ...
              <p>Faculty members with rejected IT clearance</p>
            </div>
                  ...
                  <div key={request._id} className="clearance-card rejected">
                    ...
                    <div className="faculty-info-card">
                      ...
                      <div className="rejection-details">
                        <p><strong>Rejection Date:</strong> {new Date(request.departments?.IT?.checked_at).toLocaleDateString()}</p>
                        <p><strong>Rejected By:</strong> {request.departments?.IT?.rejected_by || 'IT Staff'}</p>
                        <p><strong>Reason:</strong> {request.departments?.IT?.remarks || 'IT clearance not met'}</p>
                      </div>
        ...
        {activeTab === 'issues' && (
          <div className="issues-section">
            <div className="section-header">
              <h2><MdPushPin /> Create Issues for IT Department</h2>
            ...
            <div style={{ marginBottom: '24px' }}>
              ...
              {showIssueForm && (
                <form onSubmit={handleAddIssue} style={{
                  ...
                  >
                    {loading ? 'Creating...' : <><MdCheckCircle /> Create Issue</>}
                  </button>
                </form>
              )}
            </div>
            ...
            {departmentIssues.length > 0 ? (
              ...
              <div className="empty-state" style={{
                ...
                <p style={{ fontSize: '18px' }}><MdCheckCircle /> No pending issues for IT department</p>
              </div>
            )}
          </div>
        )}
            ...
            <div style={{ marginBottom: '24px' }}>
              ...
              {showReturnForm && (
                <form onSubmit={handleAddReturn} style={{
                  ...
                  >
                    {loading ? 'Recording...' : <><MdCheckCircle /> Record Return</>}
                  </button>
                </form>
              )}
            </div>
                ...
                {departmentReturns.map((item) => (
                  <div key={item._id} className="return-card" style={{
                    ...
                    <div className="return-details" style={{ marginTop: '12px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                      ...
                      <p style={{ margin: '0' }}><strong>Received By:</strong> {item.receivedBy || 'IT Staff'}</p>
                    </div>
                  </div>
                ))}
              </div>
