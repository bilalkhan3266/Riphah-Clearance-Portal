...
import { MdCheckCircle, MdMail, MdLogout, MdSend, MdReply, MdThumbUp, MdCancel, MdDownload, MdAttachFile, MdDescription, MdRefresh, MdPushPin, MdAssignment, MdChat, MdClose, MdCheck, MdScience } from 'react-icons/md';
import axios from 'axios';
import '../../../Departments/Phase1/Library/LibraryClearance.css';

export default function ORICClearanceEnhanced({ onComplete }) {
  ...
  const DEPARTMENT = 'ORIC';

  // Fetch clearance requests and messages on mount
  useEffect(() => {
    console.log('ORIC Component Mounted - Fetching clearance requests...');
  ...
  const fetchAllClearanceRequests = async () => {
    try {
      ...
      const response = await axios.get(`${API_URL}/api/clearance-requests?department=ORIC`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (response.data.success && response.data.requests) {
        const requests = response.data.requests;
        
        // Filter for PENDING ORIC approvals (no ORIC status or not Approved/Rejected)
        const pending = requests.filter(req => 
          !req.departments?.ORIC || 
          (req.departments?.ORIC?.status !== 'Approved' && 
           req.departments?.ORIC?.status !== 'Rejected')
        );
        setPendingRequests(pending);
        
        // Filter for APPROVED ORIC requests
        const approved = requests.filter(req => 
          req.departments?.ORIC?.status === 'Approved'
        );
        setApprovedRequests(approved);
        
        // Filter for REJECTED ORIC requests
  ...
  const handleDownloadApprovalLetter = (request, approvalType) => {
    try {
      ...
      const date = new Date(request.departments?.ORIC?.checked_at).toLocaleDateString();
...
RE: ORIC CLEARANCE APPROVAL

Dear ${facultyName},

This is to certify that your clearance request for ORIC Department has been APPROVED.

Status: APPROVED
Approved By: ${request.departments?.ORIC?.approved_by || 'ORIC Staff'}
Remarks: ${request.departments?.ORIC?.remarks || 'ORIC clearance completed successfully'}
...
RE: ORIC CLEARANCE REJECTION

Dear ${facultyName},

Your clearance request for ORIC Department has been REJECTED and requires further action.

Status: REJECTED
Rejected By: ${request.departments?.ORIC?.rejected_by || 'ORIC Staff'}
Reason: ${request.departments?.ORIC?.remarks || 'Requirements not met'}
...
Riphah International University
      ...
      link.href = url;
      link.setAttribute('download', `ORIC_${approvalType === 'approved' ? 'Approval' : 'Rejection'}_${facultyName}_${date}.txt`);
  ...
  const displayName = user?.full_name || 'ORIC Staff';
    ...
    <div className="department-page">
      <aside className="fd-sidebar">
        <div className="fd-profile">
          ...
          <div>
            ...
            <p className="fd-small">{displayRole} • ORIC Department</p>
      ...
      <main className="fd-main">
        <header className="fd-header">
          <div>
            <h1><MdScience /> ORIC Clearance Processing</h1>
        ...
        {activeTab === 'faculty-list' && (
          <div className="faculty-list-section">
            <div className="section-header">
              ...
              <p>Select a faculty to process their ORIC clearance</p>
            </div>
                ...
                <div>
                  ...
                  <div className="library-clearance-form">
                    <div className="form-header">
                      <h3><MdScience /> ORIC Clearance Processing</h3>
                      <span className="phase-badge">Phase 3 - Research Office (75%)</span>
                    </div>
                    ...
                    <div style={{
                      ...
                      <div className="button-group" style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
                        ...
                        >
                          <MdCheckCircle /> {loading ? 'Processing...' : 'Approve Clearance'}
                        </button>
                        <button 
                        ...
                        >
                          <MdCancel /> {loading ? 'Processing...' : 'Reject & Request Resolution'}
                        </button>
                      </div>
                    </div>
                  </div>
        ...
        {activeTab === 'approved' && (
          <div className="approved-section">
            <div className="section-header">
              ...
              <p>Faculty members with approved ORIC clearance</p>
            </div>
                  ...
                  <div key={request._id} className="clearance-card approved">
                    ...
                    <div className="faculty-info-card">
                      ...
                      <div className="approval-details">
                        <p><strong>Approved Date:</strong> {new Date(request.departments?.ORIC?.checked_at).toLocaleDateString()}</p>
                        <p><strong>Approved By:</strong> {request.departments?.ORIC?.approved_by || 'ORIC Staff'}</p>
                        <p><strong>Remarks:</strong> {request.departments?.ORIC?.remarks || 'ORIC clearance approved'}</p>
                      </div>
        ...
        {activeTab === 'rejected' && (
          <div className="rejected-section">
            <div className="section-header">
              ...
              <p>Faculty members with rejected ORIC clearance</p>
            </div>
                  ...
                  <div key={request._id} className="clearance-card rejected">
                    ...
                    <div className="faculty-info-card">
                      ...
                      <div className="rejection-details">
                        <p><strong>Rejection Date:</strong> {new Date(request.departments?.ORIC?.checked_at).toLocaleDateString()}</p>
                        <p><strong>Rejected By:</strong> {request.departments?.ORIC?.rejected_by || 'ORIC Staff'}</p>
                        <p><strong>Reason:</strong> {request.departments?.ORIC?.remarks || 'ORIC clearance not met'}</p>
                      </div>
        ...
        {activeTab === 'issues' && (
          <div className="issues-section">
            <div className="section-header">
              <h2><MdPushPin /> Create Issues for ORIC Department</h2>
            ...
            <div style={{ marginBottom: '24px' }}>
              ...
              >
                {showIssueForm ? 'Cancel' : '+ Create New Issue'}
              </button>
            ...
            {departmentIssues.length > 0 ? (
              ...
              <div className="empty-state" style={{
                ...
                <p style={{ fontSize: '18px' }}><MdCheckCircle /> No pending issues for ORIC department</p>
              </div>
            )}
          </div>
        )}
        ...
        {activeTab === 'returns' && (
          <div className="returns-section">
            <div className="section-header">
              <h2><MdCheckCircle /> Return Management</h2>
                ...
                {departmentReturns.map((item) => (
                  <div key={item._id} className="return-card" style={{
                    ...
                    <div className="return-details" style={{ marginTop: '12px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                      ...
                      <p style={{ margin: '0' }}><strong>Received By:</strong> {item.receivedBy || 'ORIC Staff'}</p>
                    </div>
                  </div>
                ))}
              </div>
