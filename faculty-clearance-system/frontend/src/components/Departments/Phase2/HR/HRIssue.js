...
import { MdPeople, MdAssignment, MdPendingActions, MdCheckCircle, MdSend } from 'react-icons/md';

export default function HRIssue() {
  ...
  const fetchIssues = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`/api/issues/pending/HR`);
  ...
  return (
    <div className="issue-container">
      <div className="issue-header">
        <h2><MdPeople style={{verticalAlign:'middle',marginRight:'8px'}} /> HR - Issue Files</h2>
        ...
        <form onSubmit={handleIssueItem} className="issue-form">
          ...
          <div className="form-group">
            ...
            <input
              ...
              onChange={handleFormChange}
              placeholder="Enter item name (e.g., Letter, Document)"
              required
            />
          </div>
