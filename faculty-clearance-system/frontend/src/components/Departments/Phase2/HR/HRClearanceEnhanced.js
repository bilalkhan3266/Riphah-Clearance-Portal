...
import { MdCheckCircle, MdMail, MdLogout, MdSend, MdReply, MdThumbUp, MdCancel, MdDownload, MdAttachFile, MdDescription, MdPeople, MdAssignment, MdClose, MdRefresh, MdPushPin, MdCheck } from 'react-icons/md';
import axios from 'axios';
import '../../../Departments/Phase1/Library/LibraryClearance.css';

export default function HRClearanceEnhanced({ onComplete }) {
  ...
  useEffect(() => {
    console.log('HR Component Mounted - Fetching clearance requests...');
  ...
  const fetchAllClearanceRequests = async () => {
    try {
      ...
      const response = await axios.get(`${API_URL}/api/clearance-requests?department=HR`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (response.data.success && response.data.requests) {
        const requests = response.data.requests;
        
        // Filter for PENDING HR approvals (no HR status or not Approved/Rejected)
        const pending = requests.filter(req => 
          !req.departments?.HR || 
          (req.departments?.HR?.status !== 'Approved' && 
           req.departments?.HR?.status !== 'Rejected')
        );
        setPendingRequests(pending);
        
        // Filter for APPROVED HR requests
        const approved = requests.filter(req => 
          req.departments?.HR?.status === 'Approved'
        );
        setApprovedRequests(approved);
        
        // Filter for REJECTED HR requests
  ...
  const handleDownloadApprovalLetter = (request, approvalType) => {
    try {
      ...
      const date = new Date(request.departments?.HR?.checked_at).toLocaleDateString();
...
RE: HR CLEARANCE APPROVAL

Dear ${facultyName},

This is to certify that your clearance request for HR Department has been APPROVED.

Status: APPROVED
Approved By: ${request.departments?.HR?.approved_by || 'HR Staff'}
Remarks: ${request.departments?.HR?.remarks || 'HR clearance completed successfully'}
...
RE: HR CLEARANCE REJECTION

Dear ${facultyName},

Your clearance request for HR Department has been REJECTED and requires further action.

Status: REJECTED
Rejected By: ${request.departments?.HR?.rejected_by || 'HR Staff'}
Reason: ${request.departments?.HR?.remarks || 'Requirements not met'}
...
Riphah International University
      ...
      link.href = url;
      link.setAttribute('download', `HR_${approvalType === 'approved' ? 'Approval' : 'Rejection'}_${facultyName}_${date}.txt`);
  ...
  const displayName = user?.full_name || 'HR Staff';
    ...
    <div className="department-page">
      <aside className="fd-sidebar">
        <div className="fd-profile">
          ...
          <div>
            ...
            <p className="fd-small">{displayRole} • HR Department</p>
      ...
      <main className="fd-main">
        <header className="fd-header">
          <div>
            <h1><MdPeople style={{verticalAlign:'middle',marginRight:'8px'}} /> HR Clearance Processing</h1>
        ...
        {activeTab === 'faculty-list' && (
          <div className="faculty-list-section">
            <div className="section-header">
              ...
              <p>Select a faculty to process their HR clearance</p>
            </div>
                ...
                <div>
                  ...
                  <div className="library-clearance-form">
                    <div className="form-header">
                      <h3><MdPeople style={{verticalAlign:'middle',marginRight:'8px'}} /> HR Clearance Processing</h3>
                      <span className="phase-badge">Phase 2 - Administrative Services (50%)</span>
                    </div>
        ...
        {activeTab === 'approved' && (
          <div className="approved-section">
            <div className="section-header">
              ...
              <p>Faculty members with approved HR clearance</p>
            </div>
                  ...
                  <div key={request._id} className="clearance-card approved">
                    ...
                    <div className="faculty-info-card">
                      ...
                      <div className="approval-details">
                        <p><strong>Approved Date:</strong> {new Date(request.departments?.HR?.checked_at).toLocaleDateString()}</p>
                        <p><strong>Approved By:</strong> {request.departments?.HR?.approved_by || 'HR Staff'}</p>
                        <p><strong>Remarks:</strong> {request.departments?.HR?.remarks || 'HR clearance approved'}</p>
                      </div>
        ...
        {activeTab === 'rejected' && (
          <div className="rejected-section">
            <div className="section-header">
              ...
              <p>Faculty members with rejected HR clearance</p>
            </div>
                  ...
                  <div key={request._id} className="clearance-card rejected">
                    ...
                    <div className="faculty-info-card">
                      ...
                      <div className="rejection-details">
                        <p><strong>Rejection Date:</strong> {new Date(request.departments?.HR?.checked_at).toLocaleDateString()}</p>
                        <p><strong>Rejected By:</strong> {request.departments?.HR?.rejected_by || 'HR Staff'}</p>
                        <p><strong>Reason:</strong> {request.departments?.HR?.remarks || 'HR clearance not met'}</p>
                      </div>
        ...
        {activeTab === 'issues' && (
          <div className="issues-section">
            <div className="section-header">
              <h2><MdPushPin style={{verticalAlign:'middle',marginRight:'8px'}} /> Create Issues for HR Department</h2>
            ...
            {departmentIssues.length > 0 ? (
              ...
              <div className="empty-state">
                <p><MdCheckCircle style={{verticalAlign:'middle',marginRight:'4px',color:'#10b981'}} /> No pending issues for HR department</p>
              </div>
            )}
          </div>
        )}
        ...
        {activeTab === 'returns' && (
          <div className="returns-section">
            <div className="section-header">
              <h2><MdCheckCircle style={{verticalAlign:'middle',marginRight:'4px',color:'#10b981'}} /> Record Returns for HR Department</h2>
            ...
            {departmentReturns.length > 0 ? (
              ...
              <div className="empty-state">
                <p>No returned items yet for HR department</p>
              </div>
            )}
          </div>
        )}
