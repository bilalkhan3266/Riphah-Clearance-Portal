...
import { MdCheckCircle, MdMail, MdLogout, MdSend, MdReply, MdThumbUp, MdCancel, MdDownload, MdAttachFile, MdDescription, MdFolderOpen, MdAssignment, MdClose, MdRefresh, MdPushPin, MdCheck } from 'react-icons/md';
import axios from 'axios';
import '../../../Departments/Phase1/Library/LibraryClearance.css';

export default function RecordsClearanceEnhanced({ onComplete }) {
  ...
  useEffect(() => {
    console.log('Records Component Mounted - Fetching clearance requests...');
  ...
  const fetchAllClearanceRequests = async () => {
    try {
      ...
      const response = await axios.get(`${API_URL}/api/clearance-requests?department=Records`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (response.data.success && response.data.requests) {
        const requests = response.data.requests;
        
        // Filter for PENDING Records approvals (no Records status or not Approved/Rejected)
        const pending = requests.filter(req => 
          !req.departments?.Records || 
          (req.departments?.Records?.status !== 'Approved' && 
           req.departments?.Records?.status !== 'Rejected')
        );
        setPendingRequests(pending);
        
        // Filter for APPROVED Records requests
        const approved = requests.filter(req => 
          req.departments?.Records?.status === 'Approved'
        );
        setApprovedRequests(approved);
        
        // Filter for REJECTED Records requests
  ...
  const handleDownloadApprovalLetter = (request, approvalType) => {
    try {
      ...
      const date = new Date(request.departments?.Records?.checked_at).toLocaleDateString();
...
RE: RECORDS CLEARANCE APPROVAL

Dear ${facultyName},

This is to certify that your clearance request for Records Department has been APPROVED.

Status: APPROVED
Approved By: ${request.departments?.Records?.approved_by || 'Records Staff'}
Remarks: ${request.departments?.Records?.remarks || 'Records clearance completed successfully'}
...
RE: RECORDS CLEARANCE REJECTION

Dear ${facultyName},

Your clearance request for Records Department has been REJECTED and requires further action.

Status: REJECTED
Rejected By: ${request.departments?.Records?.rejected_by || 'Records Staff'}
Reason: ${request.departments?.Records?.remarks || 'Requirements not met'}
...
Riphah International University
      ...
      link.href = url;
      link.setAttribute('download', `Records_${approvalType === 'approved' ? 'Approval' : 'Rejection'}_${facultyName}_${date}.txt`);
  ...
  const displayName = user?.full_name || 'Records Staff';
    ...
    <div className="department-page">
      <aside className="fd-sidebar">
        <div className="fd-profile">
          ...
          <div>
            ...
            <p className="fd-small">{displayRole} • Records Department</p>
      ...
      <main className="fd-main">
        <header className="fd-header">
          <div>
            <h1><MdFolderOpen style={{verticalAlign:'middle',marginRight:'8px'}} /> Records Clearance Processing</h1>
        ...
        {activeTab === 'faculty-list' && (
          <div className="faculty-list-section">
            <div className="section-header">
              ...
              <p>Select a faculty to process their records clearance</p>
            </div>
                ...
                <div>
                  ...
                  <div className="library-clearance-form">
                    <div className="form-header">
                      <h3><MdFolderOpen style={{verticalAlign:'middle',marginRight:'8px'}} /> Records Clearance Processing</h3>
                      <span className="phase-badge">Phase 2 - Documentation (50%)</span>
                    </div>
        ...
        {activeTab === 'approved' && (
          <div className="approved-section">
            <div className="section-header">
              ...
              <p>Faculty members with approved records clearance</p>
            </div>
                  ...
                  <div key={request._id} className="clearance-card approved">
                    ...
                    <div className="faculty-info-card">
                      ...
                      <div className="approval-details">
                        <p><strong>Approved Date:</strong> {new Date(request.departments?.Records?.checked_at).toLocaleDateString()}</p>
                        <p><strong>Approved By:</strong> {request.departments?.Records?.approved_by || 'Records Staff'}</p>
                        <p><strong>Remarks:</strong> {request.departments?.Records?.remarks || 'Records clearance approved'}</p>
                      </div>
        ...
        {activeTab === 'rejected' && (
          <div className="rejected-section">
            <div className="section-header">
              ...
              <p>Faculty members with rejected records clearance</p>
            </div>
                  ...
                  <div key={request._id} className="clearance-card rejected">
                    ...
                    <div className="faculty-info-card">
                      ...
                      <div className="rejection-details">
                        <p><strong>Rejection Date:</strong> {new Date(request.departments?.Records?.checked_at).toLocaleDateString()}</p>
                        <p><strong>Rejected By:</strong> {request.departments?.Records?.rejected_by || 'Records Staff'}</p>
                        <p><strong>Reason:</strong> {request.departments?.Records?.remarks || 'Records clearance not met'}</p>
                      </div>
        ...
        {activeTab === 'issues' && (
          <div className="issues-section">
            <div className="section-header">
              <h2><MdPushPin style={{verticalAlign:'middle',marginRight:'8px'}} /> Create Issues for Records Department</h2>
            ...
            {departmentIssues.length > 0 ? (
              ...
              <div className="empty-state">
                <p><MdCheckCircle style={{verticalAlign:'middle',marginRight:'4px',color:'#10b981'}} /> No pending issues for Records department</p>
              </div>
            )}
          </div>
        )}
        ...
        {activeTab === 'returns' && (
          <div className="returns-section">
            <div className="section-header">
              <h2><MdCheckCircle style={{verticalAlign:'middle',marginRight:'4px',color:'#10b981'}} /> Record Returns for Records Department</h2>
            ...
            {departmentReturns.length > 0 ? (
              ...
              <div className="empty-state">
                <p>No returned items yet for Records department</p>
              </div>
            )}
          </div>
        )}
