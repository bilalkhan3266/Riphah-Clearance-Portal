...
import { MdAccountBalance, MdAssignment, MdPendingActions, MdCheckCircle, MdSend } from 'react-icons/md';

export default function FinanceIssue() {
  ...
  const fetchIssues = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`/api/issues/pending/Finance`);
  ...
  return (
    <div className="issue-container">
      <div className="issue-header">
        <h2><MdAccountBalance style={{verticalAlign:'middle',marginRight:'8px'}} /> Finance - Issue Files</h2>
        ...
        <form onSubmit={handleIssueItem} className="issue-form">
          ...
          <div className="form-group">
            ...
            <input
              ...
              onChange={handleFormChange}
              placeholder="Enter item name (e.g., Receipt, Document)"
              required
            />
          </div>
